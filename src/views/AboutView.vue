<template>
  <div class="hello">]
    ...
    <!-- 新的电子围栏绘画 -->
    <canvas ref="tagcanvas" id="cvs1" onselectstart="return false;" style="z-index:3" width='600' height='400' @mousedown="mouseDown($event,'electricFence')"></canvas>
  </div>
</template>
    ...
    mouseDown (event) {
      if (event.button === 0 && !this.fenceFlag) {
        this.points.push({
          x:
            event.pageX -
            (this.cvs1.getBoundingClientRect().left +
              document.documentElement.scrollLeft),
          y:
            event.pageY -
            (this.cvs1.getBoundingClientRect().top +
              document.documentElement.scrollTop)
        })
    
          if (this.points.length >= 1) {
            this.clearCanvas(this.cvs1, this.ctx)
            this.drawOldSettingborder(this.points, this.ctx)
            this.cvs1.addEventListener('mousemove', this.mouseMove, false)
          }
      } else if (event.button === 2) {
        this.fenceFlag = true
        this.cvs1.removeEventListener('mousemove', this.mouseMove)
      }
    },
    // 鼠标移动时计算坐标
    mouseMove (event) {
